...
class CustomAnnData(CustomData):
    ...
    def load(self):
        ...
        self.data, self.velocity = _get_data_points(
            self.adata, self.args.embedding_name
        )
        ...
        if max_dim is not None and self.data.shape[1] > max_dim:
            print(
                f"Warning: Clipping dimensionality from {self.data.shape[1]} to {max_dim}"
            )
            self.data = self.data[:, :max_dim]
            if self.use_velocity:
                self.velocity = self.velocity[:, :max_dim]
...
def interpolate_with_ot(p0, p1, tmap, interp_frac, size):
    ...
    return np.asarray(
        [
            p0[i // len2] * (1 - interp_frac) + p1[i % len2] * interp_frac
            for i in choices
        ],
        dtype=np.float64,
    )

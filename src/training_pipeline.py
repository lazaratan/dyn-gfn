...
def train(config: DictConfig) -> Optional[float]:
    ...
    if (
        ...
        if config.model["load_pretrain"]:
            log.info(f"Instantiating model <{config.model._target_}>")
            model: LightningModule = hydra.utils.instantiate(
                config.model, dm_conf=config.datamodule, _recursive_=False
            )
            model_dict = model.state_dict()
            # print(model_dict.keys())
            # PATH = "/home/mila/l/lazar.atanackovic/structure_recovery/logs/models/per_node_linear_tcg/checkpoints/last.ckpt"
